...
# 파일 위치: src/services/nvidia_service.py -> 부모의 부모는 src/
SRC_DIR = Path(__file__).resolve().parent.parent
NVIDIA_CONFIG_PATH = SRC_DIR / "config" / "nvidia_config.yaml"
...
def _load_nvidia_config() -> dict[str, object]:
    """팀 공통 NVIDIA YAML 설정을 읽는다. 파일이 없거나 깨져도 기본값으로 돈다."""
    ...
    if not NVIDIA_CONFIG_PATH.is_file():
        return config
    try:
        loaded = yaml.safe_load(NVIDIA_CONFIG_PATH.read_text(encoding="utf-8"))
