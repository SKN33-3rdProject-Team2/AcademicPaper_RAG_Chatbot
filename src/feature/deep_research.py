...
class PaperArtifactRepository:
    ...
    def list_translated_papers(self) -> list[dict[str, Any]]:
        """번역·요약 산출물이 준비된 논문만 추출 DB 순서대로 반환한다."""
        with self._connect() as connection:
            rows = connection.execute(
                'SELECT id, title FROM "extracted" ORDER BY rowid'
            ).fetchall()
    ...
    def get_paper(self, paper_id: str) -> dict[str, Any] | None:
        """추출 DB의 서지정보와 두 Markdown의 내용을 합쳐 반환한다."""
        with self._connect() as connection:
            row = connection.execute(
                'SELECT id, title FROM "extracted" WHERE id = ?',
                (paper_id,),
            ).fetchone()
        if row is None:
            return None
        ...
        return {
            "id": str(row["id"]),
            "title": str(row["title"]),
            "translation_text": self._read_markdown(translation_path),
            "structured_summary": (
                self._read_markdown(summary_path) if has_summary else ""
            ),
...
class DeepResearchBot:
    ...
    BACK_COMMANDS = ("뒤로", "목록으로", "다른 논문", "선택 취소", "처음으로")
    ...
    def with_openai(
        ...
        *,
        ...
        return cls(repository or PaperArtifactRepository(), answerer)
    ...
    def list_papers(self) -> dict[str, Any]:
        ...
        return self._result(
            "success",
            f"번역 완료된 논문 {len(numbered)}개를 불러왔습니다.",
            papers=numbered,
            count=len(numbered),
        )
    ...
    def select_paper(self, selection: str | int) -> dict[str, Any]:
        papers = self.repository.list_translated_papers()
        if not papers:
            return self._result("empty", "선택할 번역 완료 논문이 없습니다.")
        ...
        if paper is None:
            return self._result(
                "not_found",
                "선택한 논문의 번역 내용을 DB에서 불러오지 못했습니다.",
            )
    ...
    def ask(self, question: str) -> dict[str, Any]:
        ...
        if not (
            ...
            return self._result(
                "content_missing",
                "선택한 논문에 질문에 사용할 번역문이나 요약이 없습니다.",
            )
    ...
    def as_tools(self) -> list[Any]:
        ...
        return [
            ...
            StructuredTool.from_function(
                ...
                description="현재 선택한 논문의 번역문과 요약을 근거로 질문에 답한다.",
            ),
            StructuredTool.from_function(
